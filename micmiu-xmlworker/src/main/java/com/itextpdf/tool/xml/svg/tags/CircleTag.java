/*
 * $Id: CircleTag.java 341 2012-06-18 14:38:54Z blowagie $
 *
...
//	<circle cx="60" cy="60" r="50"/>
	...
	public List<Element> end(WorkerContext ctx, Tag tag,
		...
		if(attributes != null){
			...
			float radius = 0;
			try{
				radius = Integer.parseInt(attributes.get(RADIUS));
			}catch (Exception e) {
			...
			float x = 0,  y = 0;
			try{
				x = Integer.parseInt(attributes.get(CX));
			}catch (Exception e) {
				// TODO: handle exception
			}
			
			try{
				y = Integer.parseInt(attributes.get(CY));
			}catch (Exception e) {
				// TODO: handle exception
			}
	    	...
	    	l.add(new Circle(x, y, radius, tag.getCSS()));
	    	return l;
		}else{
			return new ArrayList<Element>(0);
		}        
